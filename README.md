...
Baked detections are precomputed by our RVT-based detector — a disclosed tool run on open night data — and copied in as the incident fixture ([`contracts/fixtures/clip03.json`](contracts/fixtures/clip03.json)). They are disclosed input data, not perception built during the event; the schema, agent, app, integration, and video are what we built.
